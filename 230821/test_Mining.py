#defaultdict의 용도 : 값이 없는 경우 지정된 디폴트 값으로 대체
#OrderedDict의 용도 : 정렬을 쉽게 도와주는 도구
from collections import defaultdict, OrderedDict

# 순서) 작은 따옴표 3개 -> 그 안에 큰 따옴표 -> 해당 가사 또는 기사 혹은 문장 넣기
# lower : 소문자로 모두 변환
# split : 기본이 공백을 기준으로 단어를 분리
...
# OrderedDict 이용해서 정렬
# OrderedDict(매개변수1(정렬된 딕션너리)).items -> 키, 값을 두다 가져오는 구조
# sorted(매개변수1(집계된 딕션너리 키와 값), 매개변수2(정렬기준: 람다식), 매개변수3(기본은 오름차순으로 설정이고, reverse=True 하면 내림차순))
...
print(f"test_OrderedDict의 결과값 전체 : {test_OrderedDict}")
