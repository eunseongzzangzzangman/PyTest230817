...
# 아래 코드는 웹 상에 접근 시, 인증된 여부를 확인할 때 필요한 부분
# ssl 설정을 none 했음
# 맥북 테스, 아래 오류가 발생해서 임시 방편으로 처리한 코드
...
# 가져올 정보를 pymysql을 이용해서, 실제로 추가하는 부분
def insertData(title, artist, album, img, first):
    # con : 파이썬 <-> mysql 연결하기 위한 인스턴스, 도구
    # cur : 디비의 정보를 조작하기 위한 도구
    con, cur = None, None
    data1, data2, data3, data4 = "", "", "", ""
    sql = ""
    con = pymysql.connect(host='127.0.0.1', user='root', password='1234', database='bugs', charset='utf8')
    cur = con.cursor()
...
#   `newsImgUrl` VARCHAR(200) NULL,
    ...
    if first == True:
        cur.execute("TRUNCATE TABLE bugstb")
    ...
    try:
        ...
        # print(data5)
        # print(data6)
        query = "INSERT INTO bugstb (title, artist, album, bugsImg) VALUES (%s, %s, %s, %s)"
...
count = 1
firstYN = True
nateUrl = "https://music.bugs.co.kr/chart"
while True:
        try:
            ...
            for tag in tag_list:
                ...
                if count != 12:
                    ...
                    artist = tag.find('p', {'class' : 'artist'}).text
                    album = tag.find('a', {'class' : 'album'}).text
                    img = tag.find('img')
                    insertData(title, artist, album, img['src'], firstYN)
                    firstYN = False
                    count += 1
                else:
                    break
